...
![image-20200617161811557](md_img/image-20200617161811557.png)
...
epo等，在小程序中是无法运行的。 JSCore的环填司NodejS环境也是不尽相同，所以一些NPM的包在小程序中也是无法运
...
![image-20200617161821515](md_img/image-20200617161821515.png)
...
![img](https://pubnewfr.paperol.cn/54817726/15803617348TPsXA.png)
...
- A、
  ...
  当微信用户进入客服对话窗口时，会自动触发event类型的消息推送到云函数。

- B、

  当微信用户在对话窗口发送内容为“event”的文本消息时

- C、

  当button的open-type="feedback"，点击button时
...
当某些特定的用户操作引发事件推送时（如用户向小程序客服发送消息、或者进入会话等情况），微信服务器会将消息（或事件）的数据包以 POST 请求发送到开发者配置的 URL，开发者可以依据自身业务逻辑进行响应，比如自动回复客户的客服消息等等。

开通了云开发的小程序可以使用云函数接收消息推送，云函数目前支持哪类消息推送？
...
配置好云函数接受推送消息后，微信用户进入客服界面发送一段文本会触发哪类推送消息
...
openapi.customerServiceMessage.setTyping 下发客服当前输入状态给用户的云调用，需要在下发之前多久跟用户有交互？
...
微信用户向客服消息后，在一段时间内（目前是48小时），调用客服接口，最多可以回复几条消息给客户？
...
openapi.customerServiceMessage.setTyping 下发客服当前输入状态给用户的云调用，当command属性为typing时，微信用户的客服对话窗口标题栏会显示“对方正在输入”，如下图。如果云函数没有下发消息的话，“对方正在输入”会显示多久时间？

![IMG_0914.PNG](https://p.ananas.chaoxing.com/star3/origin/ee53a129cd88613f8051b52fe2c7bf2f.PNG)
...
openapi.customerServiceMessage.send 发送客服消息给用户的云调用，不能发送哪种类型的消息？
...
当button组件设置了session-from属性是，在哪类推送消息的云函数的event中可以获取session-from属性。
...
openapi.customerServiceMessage.uploadTempMedia 把媒体文件上传到微信服务器的云调用。目前仅支持图片。用于发送客服消息或被动回复用户消息。媒体文件上传后，几天内有效？
...
openapi.updatableMessage.createActivityId 创建被分享动态消息的activity_id的云调用。会返回activity_id的过期时间戳，默认多少小时后过期？
...
openapi.updatableMessage.setUpdatableMsg 修改被分享的动态消息的云调用。当设置 targetState为0（未开始）时，parameterList的name的必填的项目有哪些？
...
- C、
  ...
  path 点击「进入」启动小程序时使用的路径。 

- D、

  version_type 点击「进入」启动小程序时使用的版本。有效参数值为：develop（开发版），trial（体验版），release（正式版）

我的答案：AB得分： 4.5分

*2*

button组件的open-type="contact"时，如果用户在客服会话中点击下发的消息卡片后返回小程序，可以从 bindcontact 回调中获得哪些信息？
...
openapi.customerServiceMessage.getTempMedia获取客服消息内的临时素材的云调用，能获取临时素材的哪些信息？

- A、

  contentType 数据类型 (MIME Type)
...
向客服发送小程序卡片消息，需要设置button的哪些属性？![QQ20200616-040400@2x.png](md_img/4e41f0687769e79552eba3b98ec82dfa.png)
...
- B、
  ...
  send-message-title="xxx"

- C、

  send-message-path="xxxx/xxxxx"

- D、

  send-message-img="xxx/xxx"
...
openapi.updatableMessage.setUpdatableMsg 修改被分享的动态消息的云调用。parameterList的value的数据类型必须是字符串。
...
请把微信用户发送不同类型消息，触发云函数传递给云函数的event参数，连线对应。

- 第一组数据

- 1、

  [向客服发送文本消息](javascript:void(0);)

- 2、

  [向客服发送图片](javascript:void(0);)

- 3、

  [进入客服消息对话](javascript:void(0);)

- 4、

  [向客服发送小程序卡片消息](javascript:void(0);)

- 第二组数据

- A、

  [![QQ20200616-051214@2x.png](md_img/ad181442d023eafb23cc18579a7f3d89.png)](javascript:void(0);)

- B、

  [![QQ20200616-050745@2x.png](md_img/aeab046b2952a20010ab9d663ced39a6.png)](javascript:void(0);)

- C、

  [![QQ20200616-050309@2x.png](md_img/97a9b92575345a8d08a1dce764e331a8.png)](javascript:void(0);)

- D、

  [![QQ20200616-045718@2x.png](md_img/affdab8d704a1fd7b63b407f4d1fd6b9.png)](javascript:void(0);)

| 我的答案1.------------ 						    												D 																			2.------------ 						    												C 																			3.------------ 						    												A 																			4.------------ 						    												B | 得分： 4.5分 |
| ------------------------------------------------------------ | ------------ |
|                                                              |              |

*2*

请将云函数下发不同类型客户消息与截图连线
...
- 4、
  ...
  [miniprogrampage](javascript:void(0);)

- 第二组数据

- A、

  [![QQ20200616-053259@2x.png](md_img/a905100c42d3cd15f40480ccd60d956c.png)](javascript:void(0);)

- B、

  [![QQ20200616-053245@2x.png](https://p.ananas.chaoxing.com/star3/origin/8d610543e2501022bf710203235a51db.png)](javascript:void(0);)

- C、

  [![QQ20200616-053228@2x.png](https://p.ananas.chaoxing.com/star3/origin/a1d527f78e2cfc60c0a49776f1611891.png)](javascript:void(0);)

- D、

  [![QQ20200616-053503@2x.png](md_img/a492a097a693568dd83395249159b2ef.png)](javascript:void(0);)

| 我的答案1.------------ 						    												A 																			2.------------ 						    												B 																			3.------------ 						    												D 																			4.------------ 						    												C | 得分： 4.5分 |
| ------------------------------------------------------------ | ------------ |
|                                                              |              |

*3*

openapi.customerServiceMessage.send 发送客服消息给用户的云调用，下发不同类型消息的参数对应连线。
...
- A、
  ...
  [![QQ20200616-052455@2x.png](md_img/8b211321ec36ac69a24023ae14a87cec.png)](javascript:void(0);)

- B、

  [![QQ20200616-052443@2x.png](md_img/a4d1e5fdf97badb9906874d12624e481.png)](javascript:void(0);)

- C、

  [![QQ20200616-052417@2x.png](md_img/c97f9d8744453513422df6fb141a2ccf.png)](javascript:void(0);)

- D、

  [![QQ20200616-052407@2x.png](md_img/07cdc4707e6a07700318af79b3ebb8d4.png)](javascript:void(0);)

| 我的答案1.------------ 						    												D 																			2.------------ 						    												C 																			3.------------ 						    												A 																			4.------------ 						    												B | 得分： 5.5分 |
| ------------------------------------------------------------ | ------------ |
|                                                              |              |
...
- D、
  ...
  openapi.search.videoSearch

我的答案：A得分： 4.7分

*3*

openapi.pluginManager.getPluginList是查询已添加的插件的云调用。返回的结果中的pluginList属性是申请或使用中的插件列表，其中的status属性值为2的含义是？
...
openapi.wxacode.get 是获取小程序码云调用，适用于需要的码数量较少的业务场景。通过该接口生成的小程序码，永久有效，有数量限制。请求参数path是扫码进入的小程序页面路径，不能为空，最大长度是多少字节？
...
- D、
  ...
  security.audioSecCheck

我的答案：B得分： 4.7分

*6*

哪一个生成小程序二维码的服务端调用接口不能修改二维码颜色？
...
openapi.img.aiCrop是图片智能裁剪云调用,支持使用img参数实时上传图片数据，实时上传的图片尺寸限制为不超过？
...
openapi.pluginManager.applyPlugin是向插件开发者发起使用插件的申请的云调用。请求参数的action属性填写的内容是？
...
openapi.pluginManager.unbindPlugin是删除已添加的插件的云调用。参数action的内容是？
...
- A、
  ...
  operation.getJsErrSearch 错误查询

- B、

  operation.getPerformance 性能监控

- C、

  operation.getSceneList 获取访问来源

- D、

  operation.getVersionList 获取客户端版本

- E、

  operation.realtimelogSearch 实时日志查询

- F、

  operation.linkSearch 获取网页链接
...
openapi.security.imgSecCheck是校验一张图片是否含有违法违规内容的云调用接口，图片的大小限制是？
...
openapi.subscribeMessage.getPubTemplateKeyWordsById是获取模板标题下的关键词列表的云调用，返回值的data属性是关键词列表的Array，其中关键词内容对应的实例是哪个属性？
...
openapi.serviceMarket.invokeService是调用服务平台提供的服务云调用，请求参数必填的项目有？
...
- C、
  ...
  data 服务提供方接口定义的 JSON 格式的数据

- D、

  clientMsgId 随机字符串 ID，调用方请求的唯一标识

我的答案：ABCD得分： 4.7分

*2*

openapi.subscribeMessage.getPubTemplateTitleList是获取帐号所属类目下的公共模板标题的云调用，请求参数必须的属性有？
...
- A、
  ...
  接口 A: openapi.wxacode.get，适用于需要的码数量较少的业务场景生成小程序码，可接受 path 参数较长，生成个数受限。

- B、

  接口 B：openapi.wxacode.getUnlimited，适用于需要的码数量极多的业务场景，生成小程序码，可接受页面参数较短，生成个数不受限。

- C、

  接口 C：openapi.wxacode.createQRCode，适用于需要的码数量较少的业务场景

  生成二维码，可接受 path 参数较长，生成个数受限，不推荐使用。
...
OCR的云调用，需要认证公众平台账号才能调用，未认证的不能调用。

我的答案：*√*得分： 4.7分

*2*

通过后台接口可以获取小程序任意页面的小程序码，扫描该小程序码可以直接进入小程序对应的页面，所有生成的小程序码永久有效，可放心使用。 我们推荐生成并使用小程序码，它具有更好的辨识度，且拥有展示“公众号关注组件”等高级能力。云调用可以生成体验版以及发布版本的小程序的二维码
